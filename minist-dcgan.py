    ...
    with tf.Session() as sess:
        ...
        for step in range(max_steps):
            ...
            if(step%100==0):
                ...
                print(' g_loss_value :{}  d_loss_value：{} Time elapse:{}',{g_loss_value，d_loss_value，str(end_time_step - start_time_step)})
                start_time_step=end_time_step

                images = dcgan.sample_images()
                generated = sess.run(images)
                with open('pic/generated_'+str(step)+'.png', 'wb') as f:
                    f.write(generated)
        end_time = datetime.datetime.now()
        print('TOtal Time elapse: ', str(end_time - start_time))
